...
type Tab = "status" | "wallets" | "audit" | "fiat" | "peers";
...
interface SelfNodeInfo {
  voprf_public_key: string;
  ratls_available: boolean;
  peer_count: number;
}
...
function PeersTab() {
  const [self_info, setSelfInfo] = useState<SelfNodeInfo | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [info, peerList] = await Promise.all([
        adminFetch<SelfNodeInfo>("/v1/admin/peers/self"),
        adminFetch<PeerInfo[]>("/v1/admin/peers"),
      ]);
      setSelfInfo(info);
      setPeers(peerList);
    } catch (e) {
  ...
  const copyPk = useCallback(async () => {
    if (!self_info) return;
    await navigator.clipboard.writeText(self_info.voprf_public_key);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  }, [self_info]);

  const openAdd = useCallback(() => {
    setForm(EMPTY_PEER_FORM);
    setEditingId(null);
    setFormError(null);
    setShowForm(true);
  }, []);

  const openEdit = useCallback((p: PeerInfo) => {
    setForm({
      node_id: p.node_id,
      url: p.url,
      ...
      mrsigner: p.mrsigner ?? "",
      min_isv_svn: String(p.min_isv_svn),
      isv_prod_id: String(p.isv_prod_id),
    });
    setEditingId(p.node_id);
    setFormError(null);
    setShowForm(true);
  }, []);
  ...
  const submitPeer = useCallback(async () => {
    setFormError(null);
    if (!form.node_id || !form.url || !form.voprf_public_key || !form.mrenclave) {
      setFormError("Node ID, URL, VOPRF Public Key, and MRENCLAVE are required.");
      return;
    }
    if (form.mrenclave.length !== 64) {
      setFormError("MRENCLAVE must be exactly 64 hex characters (32 bytes).");
      return;
    }
    if (form.mrsigner && form.mrsigner.length !== 64) {
      setFormError("MRSIGNER must be exactly 64 hex characters (32 bytes) or empty.");
      return;
    }
    ...
    try {
      const body = {
        ...
        url: form.url,
        voprf_public_key: form.voprf_public_key,
        mrenclave: form.mrenclave,
        mrsigner: form.mrsigner || null,
        min_isv_svn: Number.parseInt(form.min_isv_svn, 10) || 0,
        isv_prod_id: Number.parseInt(form.isv_prod_id, 10) || 0,
      };
      ...
      if (editingId) {
        ...
        // POST create
        const res = await fetch("/api/proxy/v1/admin/peers", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(body),
        });
      ...
      setFormError(e instanceof Error ? e.message : "Failed to save peer");
    } finally {
      setActionLoading(false);
    }
  }, [form, editingId, load]);

  const deletePeer = useCallback(async (nodeId: string) => {
    ...
    try {
      ...
      alert(`Failed to remove peer: ${e instanceof Error ? e.message : "error"}`);
    } finally {
      setActionLoading(false);
    }
  }, [load]);

  const updateField = useCallback((field: keyof PeerForm, value: string) => {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      {self_info && (
        <div className="card" style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: "0.5rem" }}>
            ...
            <span className={`badge ${self_info.ratls_available ? "badge-success" : "badge-warning"}`}
              style={{ fontSize: "0.6875rem", marginLeft: 4 }}>
              {self_info.ratls_available ? "SGX RA-TLS" : "No RA-TLS"}
            </span>
          </div>
          <div style={{ fontSize: "0.8125rem", color: "var(--ink-secondary)", marginBottom: "0.375rem" }}>
            Share this VOPRF public key with other node operators to enable federation.
          </div>
          <div style={{
            display: "flex",
            alignItems: "center",
            ...
            <code style={{
              ...
              {self_info.voprf_public_key}
            </code>
            <button type="button" className="btn btn-ghost" onClick={copyPk}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button type="button" className="btn btn-primary" onClick={openAdd}
            style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.8125rem", padding: "0.375rem 0.75rem" }}>
      ...
      {showForm && (
        <div className="card" style={{
          ...
          <div style={{ fontWeight: 600, fontSize: "0.9375rem", color: "var(--ink)", marginBottom: "0.75rem" }}>
            {editingId ? `Edit Peer: ${editingId}` : "Add New Peer"}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem" }}>
            <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <input
                className="input" value={form.node_id}
            ...
            <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <input
                className="input" value={form.url}
                onChange={(e) => updateField("url", e.target.value)}
                placeholder="https://node-eu-1.example.com:8080"
                style={{ fontSize: "0.875rem" }}
              />
            </label>
          </div>
          ...
          <label style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: "0.75rem" }}>
            <span style={{ fontSize: "0.75rem", color: "var(--ink-secondary)", fontWeight: 500 }}>
              VOPRF Public Key (base64) *
            </span>
            <input
              className="input" value={form.voprf_public_key}
              onChange={(e) => updateField("voprf_public_key", e.target.value)}
              placeholder="Base64-encoded VOPRF public key from the peer operator"
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginTop: "0.75rem" }}>
            <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <span style={{ fontSize: "0.75rem", color: "var(--ink-secondary)", fontWeight: 500 }}>
                MRENCLAVE (64 hex chars) *
              </span>
              <input
                className="input" value={form.mrenclave}
                onChange={(e) => updateField("mrenclave", e.target.value)}
            ...
            <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              <span style={{ fontSize: "0.75rem", color: "var(--ink-secondary)", fontWeight: 500 }}>
                MRSIGNER (64 hex chars, optional)
              </span>
              <input
                className="input" value={form.mrsigner}
                onChange={(e) => updateField("mrsigner", e.target.value)}
                placeholder="Optional — pin to specific signer"
                style={{ fontSize: "0.8125rem", fontFamily: "var(--font-mono)" }}
              />
            </label>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginTop: "0.75rem" }}>
            <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <input
                className="input" type="number" min="0" value={form.min_isv_svn}
                onChange={(e) => updateField("min_isv_svn", e.target.value)}
                style={{ fontSize: "0.875rem" }}
              />
            </label>
            <label style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <input
                className="input" type="number" min="0" value={form.isv_prod_id}
                onChange={(e) => updateField("isv_prod_id", e.target.value)}
                style={{ fontSize: "0.875rem" }}
              />
            </label>
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: "1rem", justifyContent: "flex-end" }}>
            ...
            <button type="button" className="btn btn-primary" onClick={submitPeer}
              disabled={actionLoading}
              style={{ fontSize: "0.8125rem", padding: "0.375rem 0.75rem" }}>
              {actionLoading ? "Saving…" : editingId ? "Update Peer" : "Add Peer"}
            </button>
          </div>
        </div>
      )}
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{
          width: "100%", fontSize: "0.9375rem", borderCollapse: "collapse", color: "var(--ink)",
        }}>
          <thead>
            <tr style={{ borderBottom: "2px solid var(--border-strong)" }}>
              <th style={{ padding: "0.75rem 1rem", textAlign: "left", color: "var(--ink-secondary)", fontWeight: 600 }}>
                Node ID
              </th>
              <th style={{ padding: "0.75rem 1rem", textAlign: "left", color: "var(--ink-secondary)", fontWeight: 600 }}>
                URL
              ...
              <th style={{ padding: "0.75rem 1rem", textAlign: "right", color: "var(--ink-secondary)", fontWeight: 600 }}>
                Actions
              </th>
            </tr>
          </thead>
          <tbody>
            {peers.map((p) => (
              <tr key={p.node_id} style={{ borderBottom: "1px solid var(--border)" }}>
                ...
                <td style={{ padding: "0.75rem 1rem" }}>
                  <span style={{
                    fontFamily: "var(--font-mono)", fontSize: "0.8125rem", color: "var(--brand)",
                  }}>
                    {p.url}
                  </span>
                </td>
                <td style={{ padding: "0.75rem 1rem" }}>
                  <span style={{
                    fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--ink-secondary)",
                  }}>
                    {p.mrenclave.slice(0, 12)}…{p.mrenclave.slice(-8)}
                  </span>
                </td>
                <td style={{ padding: "0.75rem 1rem", textAlign: "right" }}>
            ...
            {peers.length === 0 && (
              <tr>
                <td colSpan={4} style={{
                  padding: "2rem 1rem", color: "var(--ink-muted)", textAlign: "center",
                }}>
                  No peers configured. Add a peer to enable cross-instance wallet discovery.
                </td>
              </tr>
            )}
          </tbody>
